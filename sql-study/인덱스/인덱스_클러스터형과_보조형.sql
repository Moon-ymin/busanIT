...
-- 클러스터형 인덱스의 리프 페이지 => 실제 데이터가 저장이 되어있음
...
-- 혼합된 인덱스의 경우
-- 보조 인덱스에서 컬럼명(name) 기준으로 검색하여 루트 페이지에서 리프페이지로 탐색
-- 리프 페이지(name 기준으로 정렬되어 있으며 클러스터형 인덱스의 key값을 저장)
-- 에서 이름을 기준으로 클러스터형 인덱스의 키값(userId)을 확인 한 후
