...
class ItemEventActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // 3 - 3) 어댑터 매개변수로 이벤트 람다식 추가
        binding.recyclerView.adapter = ItemAdapter(itemList) {
            Toast.makeText(this, "${it.text} 이벤트 실행",
...
class ItemAdapter(val itemList: MutableList<Item>,
    ...
    inner class ItemViewHolder(val binding: ItemBinding) : RecyclerView.ViewHolder(binding.root) {
        ...
        // 1) 뷰 홀더에서 이벤트 발생
        /*init {
            // binding.root : 항목 전체를 클릭 했을 때 이벤트 발생
            // binding.특정뷰 : 특정 항목만!!! 클릭 했을 때 이벤트 발생
    ...
    override fun onBindViewHolder(holder: ItemViewHolder, position: Int) {
        ...
        holder.itemView.setOnLongClickListener {
            ...
            //notifyItemRemoved(position)   // 삭제 알림 : position 이 변경이 안됨
            //notifyDataSetChanged()      // 전체 데이터셋 변경 알림 : position 도 변함
            true
        }
    }
}
